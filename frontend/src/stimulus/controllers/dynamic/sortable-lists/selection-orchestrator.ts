...
export class SelectionOrchestrator {
  ...
  readonly handleKeydown = (event:KeyboardEvent):void => {
    ...
    switch (event.key) {
      ...
      default:
        ...
        // Escape is deliberately absent: it is handled at the document, not
        // the root, because dropping the selection must not depend on focus
        // sitting on a card row.
        break;
    }
  };
  ...
  // Confined to the focused card's list, like range selection: "everything"
  // means everything the user is looking at, and a gesture in one list must
  // not quietly reach into the others. Selecting across lists is a later,
  // separate mechanism rather than a wider meaning of this key.
  private handleSelectAll(event:KeyboardEvent, candidate:SelectionCandidate):void {
    ...
    // a list with nothing selectable would block the browser's own
    ...
    // A fixed focused card cannot anchor the batch itself, so the first
    // orderable card of the same list stands in — never one from another
    // list, which would let a later Shift range out of the list this
    // gesture named.
  ...
  // Bound at the document, in the bubble phase, unlike every other gesture:
  // a mouse-built selection is routinely followed by focus landing on empty
  // space or the body, and Escape there still means "drop the selection".
  // Bubbling (rather than the root's capture listener) is what lets a
  // dialog's or menu's own Escape run first and be respected below.
  readonly handleEscape = (event:KeyboardEvent):void => {
    // A consumed Escape already answered the keystroke — it closed a dialog
    // or dismissed a menu — and clearing the selection off the same press
    // would take two things away for one gesture.
  ...
  // Escape belongs to the selection when it comes from inside the root or
  // from the page itself (the body, once a click on empty space dropped
  // focus). It does not when it comes from inside an overlay that owns its
  // own Escape, nor from an unrelated widget elsewhere on the page, whose
  // Escape means "dismiss that thing" — not this selection.
