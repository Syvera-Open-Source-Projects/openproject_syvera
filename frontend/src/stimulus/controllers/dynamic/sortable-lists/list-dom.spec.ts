...
describe('sortable lists DOM helpers', () => {
  ...
  describe('resolveListAppendPreviousItemId', () => {
    it('returns the last item in a list while skipping the source and truncation marker rows', () => {
      ...
      expect(resolveListAppendPreviousItemId({ excludedItemIds: new Set(['3']), rowsContainer: list })).toEqual('2');
    });
    ...
    it('returns null when the list has no other items', () => {
      ...
      expect(resolveListAppendPreviousItemId({ excludedItemIds: new Set(['1']), rowsContainer: list })).toBeNull();
    });
    ...
    it('returns null for an empty list nested inside an outer item, not the outer item\'s id', () => {
      ...
      expect(resolveListAppendPreviousItemId({ excludedItemIds: new Set(['field-1']), rowsContainer: list })).toBeNull();
    });
...
describe('itemMobility', () => {
  ...
  // Consumers that render no mobility at all keep the behaviour they had when
  // the value was two booleans defaulting to movable-and-unconfined.
  ...
  // Fails closed rather than open. The endpoint re-authorises either way, but
  // a typo must not first hand the user a draggable, selectable card that
  // only fails once the request comes back.
