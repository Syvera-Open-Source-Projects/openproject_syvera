...
describe('SelectionOrchestrator', () => {
  ...
  describe('keyboard and pointer edge cases', () => {
    ...
    it('still consumes Ctrl/Cmd+A when there is something to select', () => {
      ...
      expect(event.defaultPrevented).toBe(true);
      // Confined to the focused card's list, like a range: cards in the
      // other lists stay out.
      expect(orchestrator.selectedIds()).toEqual(['1', '2', '3']);
    });
    ...
    // Another list still holding selectable cards must not turn the key into
    // a black hole for this one: with nothing selectable here, the browser's
    // own select-all keeps the gesture.
