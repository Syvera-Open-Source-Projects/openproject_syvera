...
RSpec.describe "Backlogs batch move", :js, :selenium, :settings_reset do
  ...
  it "moves a sparse cross-list batch as one ordered block and clears the selection" do
    ...
    backlogs_page.expect_work_packages_in_sprint_in_order(
      sprint, work_packages: [sprint_wp1, bucket_wp2, sprint_wp3, sprint_wp2]
    )
  ...
  it "moves an unselected card alone, replacing the batch" do
    ...
    backlogs_page.expect_work_packages_in_sprint_in_order(
      sprint, work_packages: [sprint_wp1, sprint_wp2]
    )
  ...
  it "restores every row and keeps the selection when the server rejects the batch",
    ...
    # Not drag_work_package: that helper derives frame_reload: true from
    # cross-list identity, but a rejected move only streams an error flash
    # and never reloads the frame — the wait would time out.
    backlogs_page.drag_work_package_expecting_failure(sprint_wp2, after: bucket_wp1)

    # Rows restored, batch preserved for retry:
    backlogs_page.expect_work_packages_in_sprint_in_order(
      sprint, work_packages: [sprint_wp1, sprint_wp2, sprint_wp3]
    )
  ...
  it "keeps a same-list batch reorder without reloading the frame" do
    ...
    # A full frame reload would still settle into the expected order once it
    # finishes, so the order assertion below alone can't tell an optimistic
    # client-side move from a reload that happens to complete within the
    # wait window — see move_via_menu_spec.rb for the same reasoning. The
    # probe only flips if `#backlogs_container` actually reloads.
    backlogs_page.install_backlogs_container_reload_probe

    backlogs_page.drag_work_package(sprint_wp1, after: sprint_wp3)

    backlogs_page.expect_work_packages_in_sprint_in_order(
      sprint, work_packages: [sprint_wp3, sprint_wp1, sprint_wp2]
    )
