...
RSpec.describe "Backlogs batch selection", :js, :selenium, :settings_reset do
  ...
  describe "keyboard interaction" do
    ...
    # still means "drop the selection": it must not depend on a card row
    # holding focus.
    ...
    # List-scoped, like a range: the fixtures span a sprint and a bucket
    # precisely so a regression back to root-wide select-all would take this
    # past the sprint's four cards.
